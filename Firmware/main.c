...
#define HEXTOASCII(ch) ((ch <= 9) ? ch + '0' : ch + 55)
...
/********************************************************************
 * Function:        GetFc (unsigned long code)
 *
 * PreCondition:    None
 *
 * Input:           Raw Wiegand code
...
void AddToPutBuff (char *data, char len)
{
	char i;
	for (i = 0; i < len; i ++)
	{
		RS232_Out_Data[LastRS232Out++] = data[i];
	}
}
/********************************************************************
...
//		sCount++;
		...
		itoa (fc, buff);
		strcpy (RS232_Out_Data+sCount, buff);
		sCount += strlen (buff);
		RS232_Out_Data[sCount] = ']';
		sCount++;
		RS232_Out_Data[sCount] = ' ';
